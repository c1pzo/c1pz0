...
The C1pzo Core daemon can be configured to act as a trusted "border
router", implementing the c1pzo wire protocol and relay, making
...
The ZeroMQ feature in C1pzo Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building c1pzod:
...
For instance:
    ...
    $ c1pzod -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/c1pzod.tx.raw \
...
These options can also be provided in c1pzo.conf.
...
From the perspective of c1pzod, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into c1pzod directly. Furthermore, no information is
...
using. C1pzod appends an up-counting sequence number to each
