# PSBT Howto for C1pzo Core

Since C1pzo Core 0.17, an RPC interface exists for Partially Signed C1pzo
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/c1pzo/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for C1pzo transactions that are not fully signed
...
#### Multisig with multiple C1pzo Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
C1pzo Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `c1pzo-cli` is used.
...
Later, when *V* PZO has been received on *Amulti*, and Bob and Carol want to
