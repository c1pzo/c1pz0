C1pzo Core version 0.11.0 is now available from:

  <https://c1pzo.org/bin/c1pzo-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer PZO/kB fee to be
...
For example, add the following to `c1pzo.conf`:
...
related to the blockchain in the c1pzo system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows C1pzo Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that C1pzo Core will 
...
For an example script see [c1pzo-submittx](https://github.com/laanwj/c1pzo-submittx).
...
- #5334 `cf87536` libc1pzoconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new c1pzo-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only C1pzoAmountField
- #5753 `0f44672` Add c1pzo logo to about screen
...
- #5809 `46bfbe7` Add c1pzo-cli man page
...
- #5849 `41259ca` contrib/init/c1pzod.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename C1pzo-Qt.app to C1pzo-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for c1pzo-tx)
