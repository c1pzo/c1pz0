C1pzo Core version 0.17.0 is now available from:

  <https://c1pzocore.org/bin/c1pzo-core-0.17.0/>
...
C1pzo Core is extensively tested on multiple operating systems using
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `c1pzo.conf` file, not inside included files or from
  ...
  as c1pzo.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the c1pzo.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=c1pzo
    test.uacomment=c1pzo-testnet
...
be used in V0.17 by starting c1pzod with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed C1pzo Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/c1pzo/bips/blob/master/bip-0174.mediawiki) is an interchange format for C1pzo transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://c1pzotalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed C1pzo transaction goes through the
following steps:
...
- An **Extractor** produces a valid C1pzo transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in PZO. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When c1pzo is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If c1pzo is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart c1pzod with
...
- When running c1pzod with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If c1pzod
...
- #11962 `0a01843` add seed.c1pzo.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in PZO (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI c1pzo:// (krab)
...
- #13528 `b877c39` Move C1pzoGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libc1pzo-util to libc1pzo-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for c1pzo-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling c1pzo-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use C1pzoTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to c1pzo.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use C1pzoTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use C1pzoTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use C1pzoTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for C1pzoTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_c1pzo` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.c1pzo is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_c1pzo` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in c1pzod systemd service file (Flowdalic)
- #12652 `55f490a` c1pzo-cli: Provide a better error message when c1pzod is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to c1pzod in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for c1pzo address in translations (kaplanmaxe)
...
- #13603 `453ae5e` c1pzo-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix c1pzo-cli --version (Empact)
...
- #12877 `2b54155` Use c1pzod in Tor documentation (knoxcard)
...
- #13346 `60f0358` update c1pzo-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust c1pzocore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed c1pzo-cli -help output for help2man (hebasto)
