C1pzo Core version 0.18.0 is now available from:

  <https://c1pzocore.org/bin/c1pzo-core-0.18.0/>
...
`contrib/init/c1pzod.service`, it has been changed to use
`/var/lib/c1pzod` as the data directory instead of
`~c1pzo/.c1pzo`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/c1pzod` will
exist has enough space (check using `df -h /var/lib/c1pzod`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/c1pzo/c1pzo/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://c1pzocore.org/en/contact/) the C1pzo Core
...
The systemd init file (`contrib/init/c1pzod.service`) has been changed
to use `/var/lib/c1pzod` as the data directory instead of
`~c1pzo/.c1pzo`. This change makes C1pzo Core more consistent with
...
`datadir` option in `/etc/c1pzo/c1pzo.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/c1pzo/c1pzo.conf`.


Documentation
-------------

- A new short [document](https://github.com/c1pzo/c1pzo/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/c1pzo/c1pzo/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/c1pzo/c1pzo/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/c1pzo/c1pzo/blob/master/doc/c1pzo-conf.md)
  about the `c1pzo.conf` file describes how to use it to configure
  C1pzo Core.

- A new document introduces C1pzo Core's BIP174 [Partially-Signed
  C1pzo Transactions
  (PSBT)](https://github.com/c1pzo/c1pzo/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/c1pzo/c1pzo/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent C1pzo-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  C1pzo Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/c1pzo/c1pzo/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/c1pzo/c1pzo/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when C1pzo Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of C1pzo Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/c1pzo/c1pzo/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/c1pzo/c1pzo/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart c1pzod with the `-deprecatedrpc=generate`
...
- A new `c1pzo-wallet` tool is now distributed alongside C1pzo
...
This section describes planned changes to C1pzo Core that may affect
other C1pzo software and services.

- Since version 0.16.0, C1pzo Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with C1pzo Core 0.20 (expected about
  a year after 0.18), C1pzo Core will default to native segwit
  ...
  to bech32 addresses, and if the C1pzo Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in C1pzo Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any C1pzo Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, C1pzo Core
...
- A sub-project of C1pzo Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with C1pzo Core.  See their [project
  page](https://github.com/c1pzo-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to C1pzo Core's own implementation, although entropy
  gathered by C1pzo Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves C1pzo Core a
...
- On macOS, C1pzo Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `c1pzo_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libc1pzo_server.a` (jnewbery)
...
- #13926 [Tools] c1pzo-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like c1pzod (MarcoFalke)
...
- #14521 qt, docs: Fix `c1pzo-qt -version` output formatting (hebasto)
...
- #11625 Add C1pzoApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libc1pzo_server` (laanwj)
...
- #15154 Configure: c1pzo-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/c1pzo-wallet (Sjors)
...
- #11526 Visual Studio build configuration for C1pzo Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libc1pzo_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in c1pzo-util-test.py (isghe)
...
- #12775 Integration of property based testing into C1pzo Core (Christewart)
...
- #12246 Bugfix: Only run c1pzo-tx tests when c1pzo-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_c1pzo` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_c1pzo` (MarcoFalke)
...
- #14186 c1pzo-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_c1pzo.conf` message on startup if conf file exists (leishman)
...
- #12255 Update c1pzo.service to conform to init.md (dongcarl)
...
- #14497 Add doc/c1pzo-conf.md (hebasto)
...
- #15354 Add missing `c1pzo-wallet` tool manpages (MarcoFalke)
...
- #15765 correct c1pzoconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support c1pzo folder on OSX (JimmyMow)
