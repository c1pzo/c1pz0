...
C1pzo Core version 0.20.0 is now available from:

  <https://c1pzocore.org/bin/c1pzo-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from C1pzo Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since C1pzo Core version 0.18.0.
...
- Testing or debugging of implementations of the C1pzo P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of C1pzo Core. C1pzo Core logs debug messages
...
- OpenSSL is no longer used by C1pzo Core.  (#17265)

- BIP70 support has been fully removed from C1pzo Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/c1pzo/c1pzo/issues/16599),
  [PR #16702](https://github.com/c1pzo/c1pzo/pull/16702), and the
  `c1pzod help` for more information.  This option is experimental and
...
- All custom settings configured when C1pzo Core starts are now
...
- The "Start C1pzo Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://c1pzocore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple C1pzo Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed C1pzo Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/c1pzo-core/HWI) for
  signing.  Future versions of C1pzo Core should support a GUI option
...
- C1pzo Core's automatically-generated source code documentation is
  now available at https://doxygen.c1pzocore.org.  (#17596)
...
- The `c1pzo-cli` utility used with the `-getinfo` parameter now
...
- The `c1pzo-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `c1pzostrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_c1pzo-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBC1PZO_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_c1pzo` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBC1PZO_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for c1pzo-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate c1pzo schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest c1pzo.conf (sdaftuar)
...
- #18022 Fix appveyor `test_c1pzo` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (c1pzo-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone c1pzo-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about C1pzo Core files (hebasto)
...
- #17119 Fix broken c1pzo-cli examples (andrewtoth)
...
- #17503 Remove bitness from c1pzo-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.c1pzocore.org (laanwj)
...
- #17648 Rename wallet-tool references to c1pzo-wallet (hel-o)
...
- #18341 Replace remaining literal PZO with `CURRENCY_UNIT` (domob1812)
...
- #16115 On c1pzod startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from c1pzo-cli -getinfo (malevolent)
