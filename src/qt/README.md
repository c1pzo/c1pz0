This directory contains the source code for the C1pzo Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the C1pzo UI.
...
- ClientModel is used by the main application `c1pzogui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a c1pzo:... link from a browser).
...
* UI elements like C1pzoAmountField, which inherit from QWidget.
* `c1pzostrings.cpp`: automatically generated
* `c1pzounits.(h/cpp)`: PZO / mPZO / etc. handling
* `callback.h`
...
**Note:** Do not change `local/c1pzo_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `c1pzo-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `c1pzo-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `c1pzo-qt`
9. You're all set! Start developing, building, and debugging the C1pzo Core GUI
