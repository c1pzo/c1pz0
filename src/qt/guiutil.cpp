...
#include <qt/c1pzoaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a C1pzo address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new C1pzoAddressEntryValidator(parent));
    widget->setCheckValidator(new C1pzoAddressCheckValidator(parent));
}

bool parseC1pzoURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no c1pzo: URI
    if(!uri.isValid() || uri.scheme() != QString("c1pzo"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!C1pzoUnits::parse(C1pzoUnits::PZO, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseC1pzoURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseC1pzoURI(uriInstance, out);
}

QString formatC1pzoURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("c1pzo:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(C1pzoUnits::format(C1pzoUnits::PZO, info.amount, false, C1pzoUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openC1pzoConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", C1PZO_CONF_FILENAME));
    ...
    /* Open c1pzo.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "C1pzo.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "C1pzo (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("C1pzo (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "c1pzo.desktop";
    return GetAutostartDir() / strprintf("c1pzo-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a c1pzo.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=C1pzo\n";
        else
            optionFile << strprintf("Name=C1pzo (%s)\n", chain);
