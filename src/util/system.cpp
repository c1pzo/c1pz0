...
const char * const C1PZO_CONF_FILENAME = "c1pzo.conf";
const char * const C1PZO_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/c1pzo/c1pzo/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //c1pzo-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", C1PZO_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\C1pzo
    // macOS: ~/Library/Application Support/C1pzo
    // Unix-like: ~/.c1pzo
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "C1pzo";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/C1pzo";
#else
    // Unix-like
    return pathRet / ".c1pzo";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", C1PZO_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure C1pzo Core copyright is not removed by accident
    if (copyright_devs.find("C1pzo Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The C1pzo Core developers";
    }
