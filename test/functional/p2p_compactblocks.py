...
# TestP2PConn: A peer we use to send messages to c1pzod, and store responses.
...
class CompactBlocksTest(C1pzoTestFramework):
    ...
    # This test actually causes c1pzod to (reasonably!) disconnect us, so do this last.
    ...
    # Test that c1pzod requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for c1pzod to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # c1pzod will not send blocktxn responses for blocks whose height is
